"""
AeroCPI Cleartrip Scraper Implementation.
Implements:
...
logger = logging.getLogger("aerocpi.scraper.cleartrip")


class CleartripScraper(BaseScraper):
    """
    Cleartrip OTA scraper with anti-bot detection and seeded fallback.
    """

    def __init__(self):
        super().__init__("cleartrip")
    ...
    def fetch_quotes(
        ...
        headers["Referer"] = "https://www.cleartrip.com/"
        ...
        try:
            ...
            if response.status_code in (403, 429) or self.detect_bot_protection(content):
                logger.warning(f"[CLEARTRIP] Bot protection / rate limit on {route}. Engaging seeded fallback.")
                seeded_data = get_seeded_snapshot(route, window, source="cleartrip")
                return ScrapeResult(
            ...
            logger.error(f"[CLEARTRIP] Fetch failed for {route}: {exc}. Using seeded snapshot.", exc_info=False)
            seeded_data = get_seeded_snapshot(route, window, source="cleartrip")
            return ScrapeResult(
                source=self.source_name,
                route=route,
